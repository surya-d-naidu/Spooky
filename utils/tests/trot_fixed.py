...
T = 4500                 # full gait cycle period in milliseconds (increased for stability)
update_interval_ms = 1  # update every 20 ms for smooth motion
...
height_shift = 1.0      # how much to lower the body segment (reduce L_max)
...
# ----- Weight Shifting Function -----

    """
    Adjust the leg height based on which leg is currently swinging.
    Lower the opposite body segment to shift weight.
    """
    if not active_leg:
        return base_L
    
    # Define front and back legs
    front_legs = ['front_left', 'front_right']
    back_legs = ['hind_left', 'hind_right']
    
    # If a front leg is swinging, lower the back legs
    if active_leg in front_legs and leg_name in back_legs:
        return base_L + height_shift  # Increase L to lower the back body
    
    # If a back leg is swinging, lower the front legs
    if active_leg in back_legs and leg_name in front_legs:
        return base_L + height_shift  # Increase L to lower the front body
...
# Define servo channels for each leg and phase offsets.
    ...
    print("Starting crawl gait loop with body weight shifting. Ctrl+C to stop.")
    ...
    try:
        while True:
            ...
            # First pass: determine which leg is currently swinging
            for leg_name, params in legs.items():
                ph = params['phase']
                norm = ((t_ms + ph) % T) / T

                # Check if this leg is in swing phase
                if 0.15 <= norm < 0.35:  # swing phase (both lifting and lowering)
                    active_leg = leg_name
                    break

            # Second pass: compute each leg's state & command its servos
            for leg_name, params in legs.items():
                ph = params['phase']
                norm = ((t_ms + ph) % T) / T

                # Decide base L_desired
                if 0.15 <= norm < 0.25:
                    # swing up & forward
                    prog = (norm - 0.15) / 0.1
                ...
                # Apply weight shifting by adjusting leg height
                L_des = get_adjusted_height(leg_name, L_des, active_leg)
                ...
                # command the servos (hip servos remain static)
...
# ----- Weight Shifting Function -----
def get_adjusted_height(leg_name, base_L, active_leg):
    """
    Adjust the leg height based on which leg is currently swinging.
    Lower the opposite body segment to shift weight.
    """
