...
# Base hip angles:
HIP_FRONT = 90   # front hip base angle (degrees)
HIP_HIND  = 110  # hind (back) hip base angle (degrees)

# Define servo channels, phase offsets, and leg position for each leg.
# Phase offsets are set to alternate the stepping:
legs = {
    'left_front':  {'calf': 0,  'knee': 1,  'hip': 2,  'phase': T/2, 'position': 'front'},
    'left_back':   {'calf': 3,  'knee': 4,  'hip': 5,  'phase': 0,   'position': 'hind'},
    'right_back':  {'calf': 10, 'knee': 11, 'hip': 12, 'phase': 0,   'position': 'hind'},
    'right_front': {'calf': 13, 'knee': 14, 'hip': 15, 'phase': T/2, 'position': 'front'},
}
...
def compute_IK(L, l1, l2):
    ...
    try:
        ...
        theta = 0.0
    # Compute knee angle (using theta in radians for sin and cos).
...
def compute_delta(t, phase, T_delta, delta_amp):
    ...
    This delta is subtracted from the knee angle to produce a forward step.
    """
...
def get_hip_angle(position):
    ...
    Front legs get HIP_FRONT; hind legs get HIP_HIND.
    """
